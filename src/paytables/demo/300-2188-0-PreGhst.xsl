			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						var playBonusGame = false;
						var playMultiplierGame = false;
						...
						if (playBonusGame)
						{
							var bonusValues = getBonusValues(scenario);
							r.push('<table border="0" cellpadding="2" cellspacing="1" width="100%" class="gameDetailsTable" style="table-layout:fixed">');
							r.push('<tr>');
							r.push('<td class="tablehead" width="25%">');
 							...
 							r.push('</td>');
							r.push('<td class="tablehead" width="25%">');
	 						r.push(getTranslationByName("prizeBonus", translations));
 							r.push('</td>');
							r.push('</tr>');
 							...
 							for(var i = 0; i < bonusValues.length -1; ++i)
							{
								...
								r.push('</td>');
								r.push('<td class="tablebody">');
								r.push(convertedPrizeValues[prizeNames.indexOf(bonusValues[i])]);
								r.push('</td>');
								r.push('</tr>');
							}
							r.push('<tr>');
							r.push('<td class="tablebody">');
							r.push(getTranslationByName("Z", translations));
							r.push('</td>');
							r.push('</tr>');
						...
						if (playMultiplierGame)
						{
							var multiplierValue = getMultiplierValue(scenario);
							r.push('<table border="0" cellpadding="2" cellspacing="1" width="100%" class="gameDetailsTable" style="table-layout:fixed">');
							r.push('<tr>');
 							r.push('<td class="tablehead" width="25%">');
	 						r.push(getTranslationByName("wheelBonusPrize", translations));
 							r.push('</td>');
							r.push('<td class="tablehead" width="25%">');
	 						r.push(getTranslationByName("multiplierValue", translations));
 							r.push('</td>');
							r.push('</tr>');
							...
							r.push('<tr>');
							r.push('<td class="tablebody">');
							r.push(convertedPrizeValues[prizeNames.indexOf(multiplierValue[0])]);
							r.push('</td>');

							r.push('<td class="tablebody">');
							r.push(multiplierValue[1] + 'x');
							r.push('</td>');
							r.push('</tr>');
					...
					function getMultiplierValue(scenario)
					{
						var numsData = scenario.split("|")[3];
						return numsData.split("*");
					}
					...
					function translateOutcomeNumber(outcomeNum, translations)
					{
						if (outcomeNum == "T")
						{
							return getTranslationByName("prizeBonusTriggered", translations);
						}
						else if (outcomeNum == "U")
						{
							return getTranslationByName("wheelBonusTriggered", translations);
						}
